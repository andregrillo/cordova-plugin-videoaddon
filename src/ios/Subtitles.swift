...
extension UIViewController {
    ...
    func open(fromData data: Data, encoding: String.Encoding = String.Encoding.utf8, player:AVPlayer) {
        
        let contents = try! String(data: data, encoding: encoding) ?? ""
        show(subtitles: contents, player: player)
    }
    func open(fileFromLocal filePath: URL, encoding: String.Encoding = String.Encoding.utf8, player:AVPlayer) {
        
        let contents = try! String(contentsOf: filePath, encoding: encoding)
        show(subtitles: contents, player: player)
    }
    
    func open(fileFromRemote filePath: URL, encoding: String.Encoding = String.Encoding.utf8, player:AVPlayer) {
            ...
            DispatchQueue.main.async(execute: {
                ...
                if let checkData = data as Data? {
                    if let contents = String(data: checkData, encoding: encoding) {
                        self.show(subtitles: contents, player: player)
                    }
                }
    ...
    func show(subtitles string: String, player:AVPlayer) {
        
        // Parse
        parsedPayload = Subtitles.parseSubRip(string)
        addPeriodicNotification(parsedPayload: parsedPayload!, player: player)
        
    }
    
    func showByDictionary(dictionaryContent: NSMutableDictionary, player:AVPlayer) {
    ...
    func addPeriodicNotification(parsedPayload: NSDictionary, player:AVPlayer) {
